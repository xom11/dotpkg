...
- **`cargo check --target aarch64-pc-windows-msvc --all-targets` type-checks every `#[cfg(windows)]` path from macOS, and it works on this tree today** — measured mid-execution: three Windows targets are installed (`aarch64-pc-windows-msvc`, `x86_64-pc-windows-msvc`, `x86_64-pc-windows-gnu`) and the run compiles `windows v0.57.0` cleanly in ~3m45s cold. `cargo check` does not link, so no MSVC toolchain is needed. **Any task touching a `#[cfg(windows)]` block must run it.** This plan was written believing such code could only be verified on a14; that was wrong, and the cheaper control is now mandatory rather than optional.
  **What it does not buy, stated so nobody over-trusts it:** it catches *compile* errors on the Windows target, not *behavioural* differences. The `resolve_root` defect Phase 4's Windows run caught — `canonicalize` adding a `\\?\` prefix on Windows and not on macOS — compiles fine on both and is invisible to a cross-check. So this **adds** a control; it does not replace the Windows test runs in Task 17.
