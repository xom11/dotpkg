...
/// A package name.
///
/// Scoop resolves names case-insensitively -- comparing them any other way is
/// how `apply` removes the app it has just installed: `pkg.toml` saying `FZF`
/// against `fzf` on disk plans `Install{FZF}` and `Prune{fzf}`, and prune
/// runs last.
///
